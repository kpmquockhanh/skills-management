...
export default async (req, res) => {
  ...
  const user = await User.findById(req.user._id).select('password')
    .catch((err) => res.status(500).json(errorHelper('00070', req, err.message)));

  const match = await compare(req.body.oldPassword, user.password)
    .catch((err) => res.status(500).json(errorHelper('00071', req, err.message)));

  if (!match) return res.status(400).json(errorHelper('00072', req));

  const hashed = await hash(req.body.newPassword, 10)
    .catch((err) => res.status(500).json(errorHelper('00074', req, err.message)));

  user.password = hashed;

  await user.save().catch((err) => res.status(500).json(errorHelper('00075', req, err.message)));
