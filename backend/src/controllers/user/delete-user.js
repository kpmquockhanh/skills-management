...
import {
  generateRandomCode, errorHelper, getText, logger,
} from '../../utils/index.js';
...
export default async (req, res) => {
  ...
  }).catch((err) => res.status(500).json(errorHelper('00090', req, err.message)));

  await Token.deleteOne({ userId: req.user._id }).catch((err) => res.status(500).json(errorHelper('00091', req, err.message)));
