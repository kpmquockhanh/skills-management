...
import {
  errorHelper, getText, logger, signAccessToken, signRefreshToken,
} from '../../../utils/index.js';
...
export default async (req, res) => {
  ...
  const exists = await User.exists({ _id: req.user._id, isActivated: true })
    .catch((err) => res.status(500).json(errorHelper('00051', req, err.message)));
  ...
  ).catch((err) => res.status(500).json(errorHelper('00057', req, err.message)));
