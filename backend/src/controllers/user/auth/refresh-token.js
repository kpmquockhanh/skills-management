...
import {
  errorHelper, getText, ipHelper, signAccessToken, signRefreshToken,
} from '../../../utils/index.js';
...
export default async (req, res) => {
  ...
  const userToken = await Token.findOne({ userId: req.user._id }).catch((err) => res.status(500).json(errorHelper('00060', req, err.message)));
  ...
  ).catch((err) => res.status(500).json(errorHelper('00064', req, err.message)));
